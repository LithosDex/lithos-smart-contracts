...
contract DeployAndInitVe33Script is Script {
    ...
    function run() external {
        ...
        require(vm.exists(statePath), "State file not found. Run DeployAndInitDex.s.sol first!");
        _loadState(statePath);

        // Verify DEX contracts are deployed
        require(deployed["PairFactoryUpgradeable"] != address(0), "PairFactoryUpgradeable not deployed");
        ...
        if (activateMinter) {
            ...
            address initialMintRecipient = vm.envAddress("INITIAL_MINT_RECIPIENT");
            ve33 = DeploymentHelpers.deployVe33System(deployer, initialMintRecipient);
            ...
            DeploymentHelpers.initializeVe33(ve33, deployed["PairFactoryUpgradeable"], deployer, tokens);
        ...
        if (activateMinter) {
            ...
            console2.log("System is now active. Airdrop can proceed on Oct 12.");
            ...
            console2.log("On Oct 9, run: ACTIVATE_MINTER=true forge script script/DeployAndInitVe33.s.sol");
            console2.log("\nDeployed Addresses:");
    ...
    function _loadVe33State(string memory path) private {
        ...
        ve33.permissionsRegistry = vm.parseJsonAddress(json, ".PermissionsRegistry");
        ve33.bribeFactory = vm.parseJsonAddress(json, ".BribeFactory");
        ve33.voter = vm.parseJsonAddress(json, ".Voter");
        ve33.rewardsDistributor = vm.parseJsonAddress(json, ".RewardsDistributor");
    ...
    function _saveVe33State(string memory path) private {
        ...
        json = string.concat(json, '"Lithos":"', vm.toString(ve33.lithos), '",');
        json = string.concat(json, '"VeArtProxy":"', vm.toString(ve33.veArtProxy), '",');
        json = string.concat(json, '"VeArtProxyImpl":"', vm.toString(ve33.veArtProxyImpl), '",');
        json = string.concat(json, '"Minter":"', vm.toString(ve33.minter), '",');
        json = string.concat(json, '"MinterImpl":"', vm.toString(ve33.minterImpl), '",');
        json = string.concat(json, '"VotingEscrow":"', vm.toString(ve33.votingEscrow), '",');
        json = string.concat(json, '"GaugeFactory":"', vm.toString(ve33.gaugeFactory), '",');
        json = string.concat(json, '"PermissionsRegistry":"', vm.toString(ve33.permissionsRegistry), '",');
        json = string.concat(json, '"BribeFactory":"', vm.toString(ve33.bribeFactory), '",');
        json = string.concat(json, '"Voter":"', vm.toString(ve33.voter), '",');
        json = string.concat(json, '"RewardsDistributor":"', vm.toString(ve33.rewardsDistributor), '",');
        json = string.concat(json, '"ProxyAdmin":"', vm.toString(ve33.proxyAdmin), '",');
        json = string.concat(json, '"Timelock":"', vm.toString(ve33.timelock), '"');
