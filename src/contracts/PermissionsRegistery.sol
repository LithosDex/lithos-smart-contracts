...
/*
    This contract handles the accesses to the various Lithos contracts.
*/
...
contract PermissionsRegistry {
    ...
    /// @notice This is the Lithos team multisig 2/2
    ...
    constructor() {
        lithosTeamMultisig = msg.sender;
        lithosMultisig = msg.sender;
    ...
    modifier onlyLithosMultisig() {
        require(msg.sender == lithosMultisig, "!lithosMultisig");
        _;
    }
    ...
    function addRole(string memory role) external onlyLithosMultisig {
    ...
    function removeRole(string memory role) external onlyLithosMultisig {
        bytes memory _role = bytes(role);
    ...
    *************************************************************/
    ...
    /// @notice set Lithos team multisig
    /// @param _new new address
    function setLithosTeamMultisig(address _new) external {
        require(msg.sender == lithosTeamMultisig, "not allowed");
        require(_new != address(0), "addr 0");
        require(_new != lithosTeamMultisig, "same multisig");
        lithosTeamMultisig = _new;

        emit SetLithosTeamMultisig(_new);
    }

    /// @notice set Lithos multisig
    /// @param _new new address
    function setLithosMultisig(address _new) external {
        require(msg.sender == lithosMultisig, "not allowed");
        require(_new != address(0), "addr0");
        require(_new != lithosMultisig, "same multisig");
        lithosMultisig = _new;

        emit SetLithosMultisig(_new);
    }
}
