...
contract veNFTAPI is Initializable {
    ...
    function allPairRewards(
        ...
        for (uint256 i = 0; i < _amount; i++) {
            uint256 target = _offset + i;
            if (target >= totalPairs) {
                break;
            }
            address _pair = _pairAddressAtIndex(target);
    ...
    function setPairFactory(address _pairFactory) external {
        ...
        pairFactories.push(_pairFactory);
    ...
    function _resolvePairFactories(
        ...
        revert("veNFTAPI: factory not found");
    }
    ...
    function _pairAddressAtIndex(
        ...
        revert("veNFTAPI: pair index out of range");
    }
}
