...
contract MinterUpgradeable is IMinter, Initializable, OwnableUpgradeable {
    ...
    uint256 public weekly; // represents a starting weekly emission of 2.6M LITH (LITH has 18 decimals)
        ...
        initializer
    {
        ...
        _lithos = ILithos(IVotingEscrow(__ve).token());
        _voter = IVoter(__voter);
        _ve = IVotingEscrow(__ve);
        _rewards_distributor = IRewardsDistributor(__rewards_distributor);

        active_period = ((block.timestamp + (2 * WEEK)) / WEEK) * WEEK;
        weekly = 2_600_000 * 1e18; // represents a starting weekly emission of 2.6M LITH (LITH has 18 decimals)
        isFirstMint = true;
    }
        ...
        external
    {
        ...
        if (max > 0) {
            ...
            _lithos.approve(address(_ve), type(uint256).max);
    ...
    function calculate_rebase(uint256 _weeklyMint) public view returns (uint256) {
        uint256 _veTotal = _lithos.balanceOf(address(_ve));
        uint256 _lithosTotal = _lithos.totalSupply();

        uint256 lockedShare = (_veTotal) * PRECISION / _lithosTotal;
    ...
    function update_period() external returns (uint256) {
        ...
        if (block.timestamp >= _period + WEEK && _initializer == address(0)) {
            ...
            uint256 _balanceOf = _lithos.balanceOf(address(this));
            if (_balanceOf < _required) {
                _lithos.mint(address(this), _required - _balanceOf);
            }

            require(_lithos.transfer(team, _teamEmissions));

            require(_lithos.transfer(address(_rewards_distributor), _rebase));
            _rewards_distributor.checkpoint_token(); // checkpoint token balance that was just minted in rewards distributor
            _rewards_distributor.checkpoint_total_supply(); // checkpoint supply

            _lithos.approve(address(_voter), _gauge);
