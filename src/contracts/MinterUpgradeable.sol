...
contract MinterUpgradeable is IMinter, Initializable, OwnableUpgradeable {
    ...
    uint public weekly; // represents a starting weekly emission of 2.6M LITH (LITH has 18 decimals)
    ...
    function _initialize(
        ...
        if(max > 0){
            ...
            _lithos.approve(address(_ve), type(uint).max);
    ...
    function calculate_rebase(uint _weeklyMint) public view returns (uint) {
        uint _veTotal = _lithos.balanceOf(address(_ve));
        uint _lithosTotal = _lithos.totalSupply();
        
        uint lockedShare = (_veTotal) * PRECISION  / _lithosTotal;
    ...
    function update_period() external returns (uint) {
        ...
        if (block.timestamp >= _period + WEEK && _initializer == address(0)) { // only trigger if new week
            ...
            uint _balanceOf = _lithos.balanceOf(address(this));
