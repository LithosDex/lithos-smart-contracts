...
contract TradeHelper {
    ...
    function _calculate_deriv(uint x, uint y) internal pure returns (uint) {
        return
            (3 * y * ((x * x) / 1e18)) / 1e18 + ((((y * y) / 1e18) * y) / 1e18);
    }
    ...
    function getAmountOutStable(
        ...
        return
            (PairFactory(factory).isPair(pair))
                ? Pair(pair).getAmountOut(amountIn, tokenIn)
                : 0;
    }
    ...
    function getAmountInStable(
        ...
        if (PairFactory(factory).isPair(pair)) {
            ...
            uint reserveIn = ((
                (tokenIn == p.token0()) ? p.reserve0() : p.reserve1()
            ) * 1e18) / decimalsIn;
            uint reserveOut = ((
                (tokenOut == p.token0()) ? p.reserve0() : p.reserve1()
            ) * 1e18) / decimalsOut;
            ...
            for (uint i = 0; i < 255; i++) {
                ...
                if (new_k < old_k) {
                    uint dx = ((old_k - new_k) * 1e18) /
                        _calculate_deriv(x_1, y_1);

                    x_1 = x_1 + dx;
                } else {
                    uint dx = ((new_k - old_k) * 1e18) /
                        _calculate_deriv(x_1, y_1);
            ...
            amountIn =
                (amountInNoFees * (10000 + PairFactory(factory).getFee(true))) /
                10000;
        }
    }
    ...
    function getAmountInVolatile(
        ...
        if (PairFactory(factory).isPair(pair)) {
            ...
            uint reserveIn = (tokenIn == p.token0())
                ? p.reserve0()
                : p.reserve1();
            uint reserveOut = (tokenOut == p.token0())
                ? p.reserve0()
                : p.reserve1();

            amountIn =
                (((amountOut * reserveIn) / (reserveOut - amountOut)) *
                    (10000 + PairFactory(factory).getFee(false))) /
                10000;
        }
    }
