...
contract VoterV3 is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
        ...
        returns (address _gauge, address _internal_bribe, address _external_bribe)
    {
        ...
        // gov can create for any pool, even non-Lithos pairs
        ...
        address _owner = IPermissionsRegistry(permissionRegistry).lithosTeamMultisig();
        string memory _type = string.concat("Lithos LP Fees: ", IERC20Metadata(_pool).symbol());
        _internal_bribe = IBribeFactory(bribefactory).createBribe(_owner, tokenA, tokenB, _type);

        _type = string.concat("Lithos Bribes: ", IERC20Metadata(_pool).symbol());
    ...
    function notifyRewardAmount(uint256 amount) external {
        ...
        IERC20Metadata(base).safeTransferFrom(msg.sender, address(this), amount);
